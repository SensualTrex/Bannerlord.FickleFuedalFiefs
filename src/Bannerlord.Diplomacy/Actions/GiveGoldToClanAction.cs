...
namespace Religions.Actions
{
    internal static class GiveGoldToClanAction
    {
        ...
        {
            foreach ((var recipientHero, var amount) in MBMath.DistributeShares(gold, clan.AliveLords.Where(l => l.IsAlive && l.IsCommander), CalculateShare))
            {
        ...
        private static int CalculateShare(Hero member)
        {
            ...
            return (int)member.Clan.CurrentTotalStrength + (member == member.Clan?.Leader ? 500 : 10);
        }
